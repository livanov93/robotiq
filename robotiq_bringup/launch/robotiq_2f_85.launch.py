...
def generate_launch_description():
    ...
    robot_controllers = PathJoinSubstitution(
        [
            FindPackageShare("robotiq_bringup"),
            "config",
    ...
    gripper_action_controller_spawner = Node(
        package="controller_manager",
        executable="spawner",
        arguments=["robotiq_gripper_controller", "-c", "/controller_manager", "--stopped"],
    )
    ...
    delay_robot_controller_spawner_after_joint_state_broadcaster_spawner = RegisterEventHandler(
        event_handler=OnProcessExit(
            target_action=joint_state_broadcaster_spawner,
            on_exit=[gripper_controller_spawner, gripper_action_controller_spawner],
        )
    )
